...
| urlencoded | `username[$ne]=zzz&password=x` | `req.body.username === { $ne: 'zzz' }` ✅ | `express.urlencoded({ extended: true })` uses `qs`, see [`app.ts:48`](../server/src/app.ts#L48) |

The injection surface is **request bodies only**. The urlencoded vector is the nastier of the two, because `application/x-www-form-urlencoded` is a CORS-simple content type, so it composes with the CSRF hole in V03 and needs no preflight.
...
Read the bottom row carefully. Every one of the three *responses* to a compromise is currently broken: the user changing their password, you suspending the account, the user deleting the account. That is why V06, V10 and V17 rank higher in this queue than their individual 8/6/5 scores suggest: they are not independent bugs, they are the failure of the entire recovery path.
...
| :-: | --- | :-: | --- | --- | :-: | :-: |
| **11** | **Fail-fast validated config, then ROTATE BOTH SECRETS** | [S01](suggestions/01.fail-fast-env-config.md) → closes [V01](vulnerabilities/01.hardcoded-fallback-jwt-secrets.md) | new `server/src/config/env.ts`, delete [`dotenvHelper.ts`](../server/src/utils/dotenvHelper.ts), ~20 imports | `ACCESS_TOKEN_SECRET \|\| "fallback-access-token-secret"` is **in the repo**. If `.env` ever fails to load, from a wrong cwd, a missed dashboard variable or a fresh container, the server boots happily and signs tokens anyone can forge. **Total auth bypass, no victim interaction** | M | none |
| **12** | Cut the harvest chain: cap search + drop `email` | [V09](vulnerabilities/09.email-addresses-exposed-on-public-endpoints.md) + [V13](vulnerabilities/13.unbounded-pagination-and-unpaginated-search.md) | [`searchUserText.controller.ts:24-28`](../server/src/controllers/searchUserText.controller.ts#L24) | `GET /search/q/a` is unauthenticated, has **no limit anywhere in the file**, and returns every matching user's email. This is step ① of the attack chain. **Pulled forward from Phase 3**. It is one file and ten minutes | S | none |
...
| # | Do this | Ref | Files to touch | What is hurt right now | Effort | Needs |
| :-: | --- | :-: | --- | --- | :-: | :-: |
| **17** | **One `apiClient`** with interceptors | [S06](suggestions/06.single-http-client-with-interceptors.md) | new `client/src/lib/apiClient.ts`, ~60 hook files | 20 files use axios, 40 use `fetch`, each redeclaring the base URL and its own error handling. **Must precede CSRF**. Applying a header by hand across 60 call sites is how three of them get forgotten | M | none |
...
| **39** | Remove the dead privilege fields | [V19](vulnerabilities/19.dead-privilege-fields.md) | [`user.model.ts:48-53`](../server/src/models/user.model.ts#L48), `ModelTypes.ts`, one mongosh `$unset` | `isAdmin` is written by nothing and read by nothing, confirmed with one grep hit. It advertises which account to target. **Keep `isSuspended` and `isDeactivated`**. #21 and five existing call sites depend on them | S | 26 |
...
**Three edges are easy to get wrong, and they will bite you:**
...
| 3 | **#32, #34** | **MongoDB replica set.** Transactions require one. Atlas provides it on every tier including free; a standalone local `mongod` does not | If you are on Atlas you already have it, so verify and move on. If local, run a single-node replica set rather than gating the code |
...
**Previous:** [Audit index](00.INDEX.md) · **Next:** [Vulnerabilities guide](vulnerabilities/index.md)
