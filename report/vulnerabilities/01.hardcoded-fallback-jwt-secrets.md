...
and sends it as the `accessToken` cookie. They are now that user — any user, including one they pick by browsing `/api/v1/users/profile/:userId` (see `08.user-object-overexposure.md`, which hands out user IDs to anonymous callers). They can delete that user's videos, post as them, change their email, and read their watch history.
