...
The delete becomes one atomic `updateOne`, and the content disappears from the product immediately. The trade-off is that **every** read path must remember the filter — miss one and deleted content resurfaces. Given the codebase already has inconsistent `isPublished` filtering across six read paths (`vulnerabilities/11`), I would expect the same to happen here. Transactions are the safer choice for this codebase.
